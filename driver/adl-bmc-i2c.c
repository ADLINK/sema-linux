...
static int ec_i2c_status(struct eapi_txn *trxn)
{
	unsigned char Status;
	if (adl_bmc_ec_read_device(EC_RO_ADDR_IIC_TXN_STATUS, &Status, 1, EC_REGION_2) == 0)
	{
...
static int bmc_i2c_status (void)
{
	int i;
...
static int bmc_i2c_write (struct i2c_msg *msg)
{
	...
	if(msg->len > BMC_I2C_WRITE_LEN_MAX)  {
		...
		return bmc_i2c_status();
	}
}

static int bmc_i2c_read (struct i2c_msg *msg)
{
	unsigned char buff[32];
	buff[0] = SLAVE_ADDR(msg->addr);
...
static int bmc_i2c_write_read (struct i2c_msg *msg_wr, struct i2c_msg *msg_rd)
{
	int i, ret;
	unsigned char buff[32];
