...
static int adl_bmc_nvmem_read(void *context, unsigned int offset, void *val, size_t bytes)
{
	...
	{
	    delay(200);
	    ret = ReadMem(2, offset + i, (char*)(val + i), 32);
	    size -= 32;
	}
	else
	{
	    delay(200);
	    ret = ReadMem(2, offset + i, (char*)(val + i), size);
	    size -= size;
	}
...
static int adl_bmc_nvmem_write(void *context, unsigned int offset, void *val, size_t bytes)
{
	...
	{
	    delay(200);
	    ret = WriteMem(2, offset + i, (char*)(val + i), 32);
	    size -= 32;
	}
	else
	{
	    delay(200);
	    ret = WriteMem(2, offset + i, (char*)(val + i), size);
	    size -= size;
	}
